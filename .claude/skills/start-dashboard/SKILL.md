...
1. Call `preview_start` with `name: "dev"`. `.claude/launch.json` already
   defines it: `npm run dev`, port 3000, autoPort on. Next.js/Turbopack
   typically reports "Ready" in well under a second.
2. Verify with `preview_snapshot` (not `preview_screenshot` — the
   ...
   the page rendered).
3. If `preview_logs` shows `GET / 200`, the server is genuinely up even
