// Multi-format deliverable generation for Document Ops (Steward). Renders
...
// paragraph list rather than Steward writing differently per format.
...
import { DeliverableMeta, StewardSession } from "../types";
import { deliverablePath } from "../pdf/generate";
import StewardDocPdf from "../pdf/StewardDocPdf";
...
async function buildPdfBuffer(session: StewardSession): Promise<Buffer> {
  const doc = StewardDocPdf({ asset: session.asset, clientLabel: session.clientLabel });
  return renderToBuffer(doc);
}

async function buildWordBuffer(session: StewardSession): Promise<Buffer> {
    ...
    sections: [
      {
        children: [
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: asset.docType || "Document Ops File",
                bold: true,
                color: hex(t.primary),
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `${clientLabel} — ${asset.audience || "audience n/a"}`,
                italics: true,
                color: hex(t.muted),
...
async function buildPowerpointBuffer(session: StewardSession): Promise<Buffer> {
  const { asset, clientLabel } = session;
  const t = getActiveTokens();
  ...
  title.addText(asset.docType || "Document Ops File", {
    x: 0.5,
    y: 1.5,
    w: 9,
    fontSize: 28,
  ...
  title.addText(`${clientLabel} — ${asset.audience || "audience n/a"}`, {
    x: 0.5,
    y: 2.3,
    w: 9,
    fontSize: 14,
...
function buildEmailHtml(session: StewardSession): string {
  const { asset, clientLabel } = session;
...
<html>
  <body style="margin:0;padding:24px;background:${pageBg};font-family:${t.fontFamily};color:${t.ink};">
    <div style="max-width:600px;margin:0 auto;background:${t.paper};padding:32px;border:1px solid ${t.line};">
      ...
      <h1 style="font-size:20px;font-family:${t.headingFontFamily};margin:0 0 4px;">${escapeHtml(asset.docType || "Document Ops File")}</h1>
      <div style="font-size:12px;color:${t.muted};margin-bottom:20px;">
        ${escapeHtml(clientLabel)} — ${escapeHtml(asset.audience || "audience n/a")}
      </div>
      ${body}
    </div>
  </body>
...
export async function generateStewardDeliverable(session: StewardSession): Promise<DeliverableMeta> {
  const format = session.asset.outputFormat ?? "pdf";
  const ext = EXTENSION_BY_FORMAT[format] ?? "pdf";
  const fileName = `${session.id}-doc.${ext}`;
  ...
  return {
    fileName,
    title: session.asset.docType || "Document Ops File",
