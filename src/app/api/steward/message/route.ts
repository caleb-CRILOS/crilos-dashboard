...
import { StewardSession, OnboardingSession } from "@/lib/types";
import {
  ASSET_COMPLETE_SENTINEL,
  buildStewardDraftSystemPrompt,
  buildStewardEchoSystemPrompt,
  buildStewardOrchestratorSystemPrompt,
  DRAFT_REQUESTED_SENTINEL,
} from "@/lib/steward/prompts";
import { stewardAssetSchema, stewardAudienceSchema } from "@/lib/steward/schemas";
...
import { saveUpload, deleteUpload } from "@/lib/steward/uploads";
import { ExtractedText, extractInlineText, extractsInline } from "@/lib/uploadTextExtraction";
import { generateStewardDeliverable } from "@/lib/steward/generate";
...
type ClientContext = Parameters<typeof buildStewardOrchestratorSystemPrompt>[0];
type ExistingMaterial = { title: string; content: string }[];

function newSession(onboardingSession: OnboardingSession | undefined): StewardSession {
...
export async function POST(req: Request) {
  ...
  const prep = await mutateDb<Prep>((data) => {
    ...
    let session: StewardSession | undefined;
    if (sessionId) {
      session = data.stewardSessions.find((s) => s.id === sessionId);
      if (!session) return { kind: "error", status: 404, message: "Document Ops session not found." };
    } else {
      const onboarding = pickDefaultOnboardingSession(data.onboardingSessions);
      session = newSession(onboarding);
      data.stewardSessions.push(session);
    }
    ...
    if (isRunning(session)) {
      return {
        kind: "error",
        status: 409,
        message: "Steward is still working on your last message — give it a moment before sending another.",
      };
    }
    ...
    const existingMaterial: ExistingMaterial = onboarding?.clientId
      ? data.clientAssets
          .filter((a) => a.clientId === onboarding.clientId)
          .slice(0, 3)
          .map((a) => {
            try {
              return { title: a.title, content: readAssetFile(a.clientId, a.fileName) };
            } catch {
              return null;
            }
          })
          .filter((m): m is { title: string; content: string } => m !== null)
      : [];
  ...
  const { session, clientContext, existingMaterial, clientId, model, isKickoff } = prep;
  ...
  startTurn<StewardSession>({
    select: (data) => data.stewardSessions,
    ...
    run: async (s) => {
      const turn = await sendTurn({
        ...
        systemPrompt: buildStewardOrchestratorSystemPrompt(clientContext),
      ...
      // Atlas doesn't draft -- Steward drafts. Client-facing docs also get an
      // Echo voice-QA pass; internal-only docs skip it. Audience is read back
      // from Steward's own draft turn via a quick structured extraction.
      if (replyText.includes(DRAFT_REQUESTED_SENTINEL)) {
        const stewardTurn = await sendTurn({
          prompt: "Draft the doc now, following the brief above.",
          systemPrompt: buildStewardDraftSystemPrompt(clientContext, existingMaterial),
          resumeSessionId: latestSessionId,
          model,
        });
        latestSessionId = stewardTurn.sessionId;

        let audience: string | undefined;
        try {
          const audienceExtraction = await extractStructured({
            resumeSessionId: latestSessionId,
            schema: stewardAudienceSchema,
            model,
          });
          audience = audienceExtraction.structuredOutput?.audience as string | undefined;
        } catch {
          // Default to running Echo if we can't tell -- safer to voice-check
          // unnecessarily than skip it for a client-facing doc.
        ...
        const presentTurn = await sendTurn({
          prompt:
            "Present the finished draft above to the client now, then ask if they want a revision or if this doc is done.",
          systemPrompt: buildStewardOrchestratorSystemPrompt(clientContext),
          resumeSessionId: latestSessionId,
          model,
        });
      ...
      if (structuredOutput) {
        ...
        s.complete = true;
        try {
          s.deliverable = await generateStewardDeliverable(s);
        } catch (err) {
          console.error("[steward] deliverable generation failed:", err);
        }
        await mutateDb((_data, db) => {
          logActivity(db, {
            agent: "Steward",
            clientId,
            task: `${s.asset.docType || "Doc"} (${s.asset.audience || "unspecified"})`,
            status: "done",
          });
        });
      }
...
export async function GET(req: Request) {
  ...
  if (id) {
    const session = db.data.stewardSessions.find((s) => s.id === id);
    if (!session) return NextResponse.json({ error: "Not found" }, { status: 404 });
    return NextResponse.json({ session: withResolvedRunState(session) });
  }
  return NextResponse.json({ sessions: db.data.stewardSessions.map(withResolvedRunState) });
}
...
export async function DELETE(req: Request) {
  ...
  const removed = await mutateDb((data) => {
    const index = data.stewardSessions.findIndex((s) => s.id === id);
    if (index === -1) return undefined;
    const [session] = data.stewardSessions.splice(index, 1);
    return session;
  });
  if (!removed) {
