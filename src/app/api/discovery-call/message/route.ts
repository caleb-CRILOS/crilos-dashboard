...
import {
  ...
  buildDiscoveryCallDraftSystemPrompt,
  buildDiscoveryCallEchoSystemPrompt,
  buildDiscoveryCallOrchestratorSystemPrompt,
  DRAFT_REQUESTED_SENTINEL,
} from "@/lib/hawk/discoveryCallPrompts";
...
type ClientContext = Parameters<typeof buildDiscoveryCallOrchestratorSystemPrompt>[0];

function newSession(onboardingSession: OnboardingSession | undefined, leadLabel: string): HawkSession {
  const now = new Date().toISOString();
  return {
    id: `discovery-call-${Date.now()}`,
...
export async function POST(req: Request) {
  ...
  const leadLabel = form.get("leadLabel")?.toString().trim() || undefined;
  ...
  // Same pattern as Sales Outreach (see src/app/api/hawk/message/route.ts):
  // txt/md/docx get extracted server-side and inlined; images/PDF get a
  // scratch copy the background turn's CLI reads, cleaned up by startTurn.
  ...
  const prep = await mutateDb<Prep>((data) => {
    ...
    if (sessionId) {
      session = data.hawkSessions.find((s) => s.id === sessionId && s.leadLabel);
      if (!session) return { kind: "error", status: 404, message: "Discovery Call lead not found." };
    } else {
      if (!leadLabel) {
        return { kind: "error", status: 400, message: "A lead label is required to start a new lead." };
      }
      const onboarding = pickDefaultOnboardingSession(data.onboardingSessions);
      session = newSession(onboarding, leadLabel);
    ...
    if (isRunning(session)) {
      return {
        kind: "error",
        status: 409,
        message: "This prep is still being worked on — give it a moment before sending another message.",
      };
    }
  ...
  startTurn<HawkSession>({
    ...
    run: async (s) => {
      const turn = await sendTurn({
        ...
        systemPrompt: buildDiscoveryCallOrchestratorSystemPrompt(clientContext),
      ...
      // Atlas doesn't draft -- Hawk drafts, Echo reviews Hawk's draft, then
      // Atlas presents Echo's version. Each is a separate resumed CLI call.
      if (replyText.includes(DRAFT_REQUESTED_SENTINEL)) {
        const hawkTurn = await sendTurn({
          prompt: "Draft the prep sheet now, following the brief above.",
          systemPrompt: buildDiscoveryCallDraftSystemPrompt(clientContext),
        ...
        const echoTurn = await sendTurn({
          ...
          systemPrompt: buildDiscoveryCallEchoSystemPrompt(clientContext),
          resumeSessionId: latestSessionId,
          model,
        });
        ...
        const presentTurn = await sendTurn({
          prompt:
            "Present Echo's finished prep sheet above to the coach now, then ask if they want a revision or if this prep sheet is done.",
          systemPrompt: buildDiscoveryCallOrchestratorSystemPrompt(clientContext),
          resumeSessionId: latestSessionId,
          model,
        });
      ...
      if (structuredOutput) {
        ...
        try {
          ...
          console.error("[discovery-call] deliverable generation failed:", err);
        }
        await mutateDb((_data, db) => {
          logActivity(db, {
            agent: "Hawk",
            clientId,
            task: `Discovery call prep: ${s.leadLabel}`,
            status: "done",
          });
        });
      }
...
export async function PATCH(req: Request) {
  ...
  const session = await mutateDb((data) => {
    const s = data.hawkSessions.find((x) => x.id === sessionId && x.leadLabel);
    if (!s) return undefined;
    s.stage = "Call Completed";
    s.updatedAt = new Date().toISOString();
    return structuredClone(s);
  });
  if (!session) {
    return NextResponse.json({ error: "Discovery Call lead not found." }, { status: 404 });
  }
...
export async function GET(req: Request) {
  ...
  if (id) {
    const session = db.data.hawkSessions.find((s) => s.id === id && s.leadLabel);
    if (!session) return NextResponse.json({ error: "Not found" }, { status: 404 });
    return NextResponse.json({ session: withResolvedRunState(session) });
  }
  return NextResponse.json({
    sessions: db.data.hawkSessions.filter((s) => s.leadLabel).map(withResolvedRunState),
  });
}
...
export async function DELETE(req: Request) {
  ...
  const removed = await mutateDb((data) => {
    const index = data.hawkSessions.findIndex((s) => s.id === id && s.leadLabel);
